...
return {
    ...
    config = function()
      ...
      vim.keymap.set('n', '<leader>ff', fzf.blines, { desc = '[F]ind [F]uzzily in current buffer' })
      vim.keymap.set('n', '<leader>fd', fzf.lsp_document_diagnostics, { desc = '[F]ind lsp [D]iagnostics' })
